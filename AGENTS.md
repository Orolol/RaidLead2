...
- **LSP GDScript** : Plugin `Codex-gdscript` pour diagnostics temps réel, go-to-definition, completions, hover sur fichiers `.gd`.

### Workflow recommandé pour toute tâche de code
1. Lire ce AGENTS.md pour le contexte.
...
## Exécution sous Windows natif (poste actuel)

Le développement courant se fait aussi sous **Windows natif** avec le binaire console Godot 4.6.2.
`project.godot` est en `rendering_method="mobile"` + `GL Compatibility` : garder `--rendering-driver opengl3` reste sans danger et homogène avec WSL.

- Binaire : `C:\Users\gaeta\Downloads\Godot_v4.6.2-stable_win64.exe\Godot_v4.6.2-stable_win64_console.exe`
- Tests (comportement) : `& $godot --rendering-driver opengl3 --headless --path . "res://tests/TestRunner.tscn"`
- **Validation de syntaxe** : `--check-only` peut rester **suspendu** sous Windows. Utiliser à la place
  `& $godot --rendering-driver opengl3 --headless --path . "res://tests/CheckScripts.tscn"` (charge tous les
  scripts via `load()` et sort 0/1 — terminant, autoloads disponibles).
- Sous Windows, `--headless` est OK pour les tests (pas de screenshots requis) ; `xvfb-run` ne concerne que WSL.
