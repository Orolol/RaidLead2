...
func show_notification(text: String, type: NotificationType = NotificationType.INFO, duration: float = DEFAULT_DURATION, title: String = "", icon: String = "") -> void:
...
func _display_notification(notification_data: Dictionary) -> void:
	...
	var notification_toast: Control = _create_notification_toast(notification_data)
...
func _position_notification(toast: Control, index: int) -> void:
	"""Positionne une notification selon son index"""
	var viewport_size: Vector2 = get_viewport().get_visible_rect().size

	var x: float = viewport_size.x - NOTIFICATION_WIDTH - NOTIFICATION_MARGIN_RIGHT
	var y: float = NOTIFICATION_MARGIN_TOP + index * (NOTIFICATION_HEIGHT + VERTICAL_SPACING)

	toast.position = Vector2(x, y)

func _animate_notification_in(toast: Control) -> void:
	"""Animation d'apparition de la notification"""
	var original_pos: Vector2 = toast.position
...
func _animate_notification_out(toast: Control, callback: Callable) -> void:
...
func _remove_notification(notification_instance: Dictionary) -> void:
...
func _on_progression_updated(phase, progress) -> void:
...
func _on_member_connected(member) -> void:
	"""Quand un membre se connecte"""
	show_info("%s s'est connecté." % member.nom, "Connexion")

func _on_member_disconnected(member) -> void:
...
func _on_dungeon_ended(dungeon_instance) -> void:
...
func _on_random_event_triggered(event) -> void:
