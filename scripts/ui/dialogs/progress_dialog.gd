...
func set_show_percentage(show: bool):
	...
	if percentage_label:
		percentage_label.visible = show

func set_show_cancel_button(show: bool):
	show_cancel_button = show
	if cancel_button:
		cancel_button.visible = show

func set_show_details(show: bool):
	show_details = show
	if is_inside_tree():
		_setup_progress_dialog()
...
func set_progress(progress: float, text: String = ""):
...
func set_progress_percentage(percentage: float, text: String = ""):
	"""Met à jour le progrès (0 à 100)"""
	set_progress(percentage / 100.0, text)

func add_progress(delta: float, text: String = ""):
	"""Ajoute au progrès actuel"""
	set_progress(current_progress + delta, text)

func add_detail(detail: String, color: Color = Color.WHITE):
...
func complete_operation(final_text: String = "Terminé !"):
...
func set_callbacks(on_cancel: Callable = Callable(), on_complete: Callable = Callable(), on_progress: Callable = Callable()):
...
func update_time_display():
	...
	var elapsed = get_elapsed_time()
	var remaining = get_estimated_time_remaining()
	
	var time_text = "Temps écoulé: %s" % _format_time(elapsed)
...
func connect_to_http_request(http_request: HTTPRequest):
...
func connect_to_worker_thread(worker_thread: Thread, progress_callback_func: Callable):
...
func setup_for_dungeon_simulation():
