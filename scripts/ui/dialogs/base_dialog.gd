...
func set_dialog_title(title: String):
	dialog_title = title
	if title_label:
		title_label.text = title

func set_dialog_size(new_size: Vector2):
	dialog_size = new_size
	size = new_size

func set_modal(is_modal: bool):
	modal = is_modal
	if is_inside_tree():
		set_flag(Window.FLAG_POPUP, modal)
		popup_window = modal

func set_resizable(can_resize: bool):
	resizable = can_resize
	unresizable = not can_resize

func set_closable(can_close: bool):
	closable = can_close
	if close_button:
...
func close_dialog(dialog_result: Dictionary = {}):
...
func confirm_dialog(dialog_result: Dictionary = {}):
...
func add_standard_buttons(show_ok: bool = true, show_cancel: bool = true, ok_text: String = "OK", cancel_text: String = "Cancel"):
...
func set_callback(action: String, callback: Callable):
...
func _apply_button_style(button: Button, style: ButtonStyle):
...
func center_on_control(control: Control):
...
func set_minimum_size_auto():
	"""Définit la taille minimale automatiquement"""
	await get_tree().process_frame
	var content_size = content_area.get_combined_minimum_size()
	var title_height = title_bar.size.y if title_bar else 40
...
static func show_message(title: String, message: String, parent: Node = null):
	"""Affiche un message simple"""
	var dialog = create_simple_dialog(title, message, parent)
	dialog.open_dialog()

static func show_confirmation(title: String, message: String, callback: Callable, parent: Node = null):
...
func set_result_data(key: String, value):
